<template>
	<view class="order-detail" v-if="calcOrderMsg">
		...
		<view class="back">
			<img src="../../static/images/user/back.png" alt="" class="back-img" @click="handleBack">
		</view>
		<view class="address container" @click="go('/user/site/site-manage?appoint=true')">
			...
			<view class="address-detail" v-else>
				<view class="detailedAddress">{{ defaultAddress.detailedAddress }}</view>
				<view class="contact">{{ defaultAddress.name }} {{ defaultAddress.mobile }}</view>
			</view>
			<view class="location"><img src="../../static/images/icon/location-logo.png" alt="" class="location-logo"></view>
		...
		<view class="order container" v-if="orderInfo">
			...
			<view class="words">
				...
				<textarea v-model.trim="opForm.message" placeholder="可在此给留言给商家" id="" cols="2" maxlength="20" placeholder-class="input-text"></textarea>
			</view>
		...
		<CouponPopup @confirm="handleConfirmChooseCoupon" :cartId="cartId" v-model="couponPopupVisible"></CouponPopup>
	</view>
</template>
