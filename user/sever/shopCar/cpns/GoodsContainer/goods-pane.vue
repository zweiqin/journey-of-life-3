	...
	<view class="goods-pane" v-if="data">
		<view class="brand">
			<view class="brand-info">
				<tui-icon v-show="currentMode === ON_EDIT" @click="handleChangeAllBrandGoodsStatus"
					:color="brandTotalCheck ? '#FFC117' : ''" :size="25" :name="brandTotalCheck ? 'circle-fill' : 'circle'"></tui-icon>
				<tui-icon v-show="currentMode === ON_CONFIRM" :color="deleteAllGoods ? 'red' : ''" :size="25"
					@click="handleAddAllBrandGoodDelete" :name="deleteAllGoods ? 'circle-fill' : 'circle'"></tui-icon>
			...
			<view class="goods-list">
				<view class="item" v-for="goods in data.cartList" :key="goods.id">
					<tui-icon v-show="currentMode === ON_EDIT" :size="25" @click="handleChangeGoodsStatus(goods)"
						:color="goods.checked ? '#FFC117' : ''" :name="goods.checked ? 'circle-fill' : 'circle' "></tui-icon>
					<tui-icon v-show="currentMode === ON_CONFIRM" :size="25"
						:color="deleteList.includes(goods.productId) ? 'red' : ''" @click="handleDeleteGoods(goods)"
						:name="deleteList.includes(goods.productId) ? 'circle-fill' : 'circle'"></tui-icon>
					<view class="goods-info">
						...
						<view class="goods-wrapper">
							...
							<view class="goods-op">
								...
								<uni-number-box @change="handleChangeGoodsNumber($event, goods)" :min="0"
									:value="goods.number"></uni-number-box>
							</view>
						</view>
					</view>
				</view>
