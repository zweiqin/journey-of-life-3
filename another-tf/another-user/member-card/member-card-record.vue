<template>
	<view class="member-card-record-container">
		<JHeader title="会员卡收支明细" width="50" height="50"></JHeader>
			...
			<view style="margin-top: 20rpx;">
				<view v-if="[ 1 ].includes(queryType)">
					<view v-if="accountingInfo.data.length > 0">
						<view
							v-for="(item, index) in accountingInfo.data" :key="item.id" style="padding-bottom: 20rpx;"
						>
							<ATFMemberCardAccount :data="item" type="entry">
				...
				<view v-else-if="[ 2 ].includes(queryType)">
					<view v-if="outgoingInfo.data.length > 0">
						<view
							v-for="(item, index) in outgoingInfo.data" :key="item.id" style="padding-bottom: 20rpx;"
						>
							<ATFMemberCardAccount :data="item" type="outgoing">
							</ATFMemberCardAccount>
						</view>
					</view>
					<view style="padding-bottom: 45rpx;">
...
<script>
import { getOutgoingRecordShopMemberCardApi, getEntryRecordShopMemberCardApi } from '../../../api/anotherTFInterface'
export default {
	...
	methods: {
		getAccountingLogsList(isLoadmore) {
			uni.showLoading()
			getEntryRecordShopMemberCardApi({ ...this.accountingInfo.query })
				.then((res) => {
		...
		getOutgoingLogsList(isLoadmore) {
			uni.showLoading()
			getOutgoingRecordShopMemberCardApi({ ...this.outgoingInfo.query })
				.then((res) => {
