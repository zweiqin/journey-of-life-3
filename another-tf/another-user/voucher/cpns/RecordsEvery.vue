		...
		<view class="MakeMoneyRecordsList">
			<scroll-view scroll-y="true" class="scrollY" @scrolltolower="getMore">
				<view v-for="(item, index) in dataList" :key="index" class="listItem">
					...
					<text class="txt2" style="display: flex;">
						<text v-if="((showType == 'income') && [4, 7].includes(item.type)) || ((showType == 'expenditure') && [2, 4].includes(item.type))">-</text>
						<text v-else>+</text> {{ item.number }}
					</text>
