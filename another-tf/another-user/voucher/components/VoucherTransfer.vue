<template>
	<view class="voucher-transfer-container" :style="{ margin }">
		<view class="operation-btn">
			<tui-button type="gray" :size="36" width="280rpx" height="78rpx" margin="0 auto" @click="isShowPopup = true">
				用户转赠
			</tui-button>
		</view>
...
<script>
import { updateOrderUserToUserTransferApi, updateUserToUserTransferApi, updateOrderUToShopTransferApi, updateUserToShopTransferApi, getBandUserInfoApi } from '../../../../api/anotherTFInterface'
		...
		handleTransfer() {
			...
			if (this.transferForm.voucherTarget === '1') {
				if (this.transferForm.voucherType === '1') {
					_url = updateOrderUserToUserTransferApi
				} else if (this.transferForm.voucherType === '2') {
					_url = updateUserToUserTransferApi
				}
			} else if (this.transferForm.voucherTarget === '2') {
				if (this.transferForm.voucherType === '1') {
					_url = updateOrderUToShopTransferApi
				} else if (this.transferForm.voucherType === '2') {
					_url = updateUserToShopTransferApi
				}
			}
