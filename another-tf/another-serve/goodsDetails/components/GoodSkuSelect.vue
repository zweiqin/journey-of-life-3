    ...
    <tui-bottom-popup :show="isShowDetails" @close="isShowDetails = false">
      <view class="goosDetailshow-box">
        <view class="detailImg-box flex-row-plus">
          <image
            class="detailImg"
            :src="common.seamingImgUrl(selectedSku.image)"
          ></image>
          <view class="flex-column-plus mar-left-40">
            <view class="font-color-C5AA7B">
              ...
              <label
                ...
                v-text="
                  selectedSku.activityType === 1 && btnType === 4
                    ? selectedSku.salePrice
                    : selectedSku.price
                "
              ></label>
            </view>
            <view class="fs24 font-color-999 mar-top-20"
              >库存 {{ selectedSku.stockNumber }} 件</view
            >
              ...
              >
                ...
                {{
                  selectedSku.price
                    ? `${
                        (Number.parseFloat(
                          selectedSku.presenterVoucher / selectedSku.price
                        ).toFixed(3) *
                          1000) /
                        10
                      }%`
                    : selectedSku.presenterVoucher
                }}
                代金券
              </view>
            </view>
        ...
        >
          <scroll-view scroll-y style="height: 100%; max-height: 50vh">
            <view
              v-for="nameItem in goodsDetail.names"
              :key="nameItem.nameCode"
            ...
            >
              ...
              >
                <view
                  v-for="tag in nameItem.values"
                  :key="tag.valueCode"
                  style="
                    background-color: #ffffff;
                    margin: 24rpx 15upx 0;
                    padding: 10upx 32upx;
                  "
                  :style="{
                    boxShadow:
                      selectedAttr[nameItem.nameCode] === tag.valueCode
        ...
        <view class="goodsNumCent">
          <view class="goodsNum-box flex-row-plus flex-sp-between">
            ...
            <view class="goodsNum">
              <view class="item subtract" @click="handleNumSub"> - </view>
              <view class="item goodsNumber">
                {{ number }}
              </view>
              <view class="item add" @click="handleNumAdd"> + </view>
            </view>
          </view>
        </view>
...
export default {
  ...
  onShow() {
    ...
    this.handleSelectBySkuId(this.skuId);
  },
  data() {
    return {
  ...
  methods: {
        ...
        if (this.goodsDetail.map[skuValueCodeItem].skuId === skuId) {
          ...
          this.goodsDetail.names.forEach((nameItem) => {
            nameItem.values.some((tag) => {
              if (
                this.goodsDetail.map[skuValueCodeItem].valueCodes
                  .split(",")
                  .includes(tag.valueCode)
              ) {
    ...
    handleClickSkuItem(nameCode, tagItem) {
      ...
      const { goodsDetail, selectedAttr } = resolveGoodsDetailTagsSituation(
        this.goodsDetail,
        this.selectedAttr,
        nameCode,
        tagItem
      ...
      this.selectedSku =
        Object.values(this.goodsDetail.map).find((skuItem) =>
          skuItem.valueCodes
            .split(",")
            .every((nameCodeItem) =>
              Object.values(this.selectedAttr).includes(nameCodeItem)
            )
        ) || {};
      if (this.selectedSku.skuId)
        this.$emit("current-select-sku", {
          selectedSku: this.selectedSku,
          currentSku: this.getCurrentSkuName(),
          number: this.number,
        });
    },
    ...
    getCurrentSkuName() {
      if (this.selectedSku.valueCodes) {
        ...
        this.goodsDetail.names.forEach((nameItem) => {
          nameItem.values.some((tag) => {
            if (
              this.selectedSku.valueCodes.split(",").includes(tag.valueCode)
            ) {
    ...
    async handleAddCart() {
      ...
      if (this.selectedSku.stockNumber < 1)
        return this.$showToast("该商品库存不足");
      if (
        this.selectedSku.stockNumber &&
        this.number > this.selectedSku.stockNumber
      )
        return this.$showToast("已超出最大数量限制");
      uni.showLoading();
      try {
          ...
          this.$emit(
            ...
            res.data.reduce(
              (total, value) =>
                total +
                value.skus.reduce(
                  (t, v) => t + (v.shelveState ? v.number : 0),
                  0
                ),
              0
            )
