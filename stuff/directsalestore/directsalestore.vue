<template>
	<view class="directsalestore">
		...
		<view class="info">
			<img class="bgm" :src="picUrl || require('../../static/images/stuff/goods1.png')" alt="" />

			<tui-icon
				name="arrowleft" color="#ffffff" :size="36" unit="upx"
				style="padding: 6upx;;background-color: #777778;border-radius: 50%;" @click="handleBack"
			></tui-icon>
		...
		<view class="main">
					...
					<view class="address">
						...
						<view class="bot">
							<view class="map">
								...
								<image src="../../static/images/stuff/global-fill.png" mode="scaleToFill" class="icon" />
							</view>
							<view class="ask">
								<view class="text">在线咨询</view>
								<image src="../../static/images/stuff/message-3-fill.png" mode="scaleToFill" class="icon" />
							</view>
							<view class="ops">
								<view>
									到这里去
			...
			<view ref="navsRef" class="navs1">
				<view
					v-for="sub in navs1" :key="sub.value" class="sub" :class="{ active: sub.value == current }"
			...
			<view v-if="current == 1" ref="navs2Ref" class="navs2">
				<view
					v-for="item in catalogList" :key="item.id" class="item" :class="{ active: item.id == currentTab }"
			...
			<view v-if="current == 1" class="goods-wrapper">
				<Goods
			...
			<view
				v-if="current !== 1" class="white" style="
          display: flex;
...
import { navs2, navs1 } from '../../pages/stuff/config'
