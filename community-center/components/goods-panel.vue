	...
	<view class="goods-panel" :style="{
      'padding-bottom': isCollapse ? '80upx' : '',
		...
		<view v-show="isCollapse">
			<Field class="item" v-for="item in goodsPanelInfo" :data="item" :key="item.label"
				v-model="goodsItem[item.field]">
				<template v-if="item.field === 'goodsUrl'" v-slot:row>
					<img @click="previewImg(index)" v-for="(item, index) in goodsItem['goodsUrl']" :key="item"
						:src="item" class="upload-img" />
					<Upload class="upload-icon" @after-read="afterRead" max-count="3" :deletable="false" />
				</template>

				<template v-if="item.field === 'goodsType'">
					<picker mode="multiSelector" range-key="name" :value="commodityTypeIndex"
						:range="commodityTypeInitRange" @columnchange="typeColumnChange" @change="typeChange">
						<view :style="{ color: goodsItem.goodsType ? '#000' : '' }" class="goodsType">
	...
	export default {
		...
		methods: {
			...
			async init() {
				...
				console.log("最终章", this.commodityTypes);
