<template>
  <view class="vip-page">
    ...
    <VipHeader
      ...
      :name="
        type == 2
          ? `${myAttributeVipJInData.serverName || '金管家会员'}`
          : `(${currentJUHUIdata.serverName || '限时钜惠'})`
      "
    ...
    <view v-if="type == 2" class="pane">
      <view class="desc-pane">
        <view class="text">
          <view class="text1"
            >开通{{ myAttributeVipJInData.serverName || "金管家会员" }}</view
          >
    ...
    <VipServePane
      ...
      :sub-title="
        type == 2
          ? '金管家会员权益'
          : `${currentJUHUIdata.serverPrice}元${
              currentJUHUIdata.serverName || '限时钜惠'
            }`
      "
    ...
    <view v-if="type == 2" class="gift">
      <image
        class="gift-icon"
        src="../../static/images/con-center/vip/gift.png"
      ></image>
      <text>购买本套餐赠送羊驼一只</text>
      <image
        class="gift-detail"
        src="../../static/images/con-center/vip/gift-detail.png"
      ></image>
    </view>
    ...
    <!-- 权益 -->
    <view class="tip"
      >温馨提示：在法律允许范围内，本公司拥有该活动最终解释权</view
    >
...
import { getPurchaseRecordApi, getCreateCodeApi } from "../../api/user";
...
export default {
  ...
  data() {
    return {
      ...
      activityCode: "",
      qrcodeUrl:
        "https://www.tuanfengkeji.cn/TFShop_Uni_H5/#/user/sever/activityCenter/index?code=",
  ...
  methods: {
    ...
    async queryDynamicData() {
      ...
      const res = await queryDynamicDataApi({
        address:
          currentDetail && JSON.stringify(currentDetail) != "[]"
            ? currentDetail
            : "",
        correspondType: 2,
      });
    ...
    addData(res, isCustom) {
      ...
      if (this.type == 2) {
        ...
        if (isCustom && this.indulgenceData.length) {
          const serveContentList =
            this.indulgenceData[0].serverContent.split(",");
          ...
          console.log("日你妈", this.indulgenceData);
    ...
    async handleShareActivity() {
      if (this.campaignsType === 0) {
        const res = await getPurchaseRecordApi({
          userId: getUserId(),
          price: 299,
        }); // TODO: 为啥是299
        if (res.data) {
      ...
      if (this.campaignsType === 0) {
        ...
        const failObj = {
          data: {
            ...
            imageUrl:
              "https://www.tuanfengkeji.cn/TFShop_Uni_H5/static/images/con-center/alpaca-forward-300.png",
          },
          successCb: () => {},
          failCb: () => {},
        };
...
.vip-page {
  ...
  background: url("../../static/images/con-center/vip/vip-pabe-bg.png")
    no-repeat;
