...
export const orderInfoFieldMap = [
  {
    label: '订单编号',
    field: 'orderNo',
    op: {
      label: '复制',
      handler: (orderDetailInfo, opsuccess, onfail) => {
        uni.setClipboardData({
          data: orderDetailInfo.orderNo,
          showToast: false,
          success: function () {
            opsuccess && typeof opsuccess === 'function' && opsuccess();
          },
          fail: function (error) {
  ...
  },
  {
    label: '服务地址',
    field: 'dictName',
    format(orderDetailInfo) {
      return orderDetailInfo.consigneeAddress + ' - ' + orderDetailInfo.consigneeAddressDetail;
    }
  },
  {
    label: '实付金额',
