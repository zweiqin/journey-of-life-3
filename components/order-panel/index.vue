<template>
	<view class="order-panel">
		...
		<view class="status" :style="statusStyles">{{ status ? '交易完成' : '交易待完成' }}</view>
		<!-- 客户信息 -->
			...
			<view class="line" v-for="item in baseInfo" :key="item.id">
				...
				<view class="text-value">
					...
					<template v-if="item.id === 1">
						<view style="color: #ff8f1f" class="text">08月18日（周四）19：00</view>
					</template>
					<template v-if="item.id === 2">
						<view class="text" style="color: #07b9b9">{{ defaultAddress ? defaultAddress.detailedAddress : '请选择地址' }}</view>
					</template>
				</view>
			</view>
		...
		<view v-else class="item flex" @click="handleChooseAddress" style="justify-content: space-between">
			请选择收货地址
      ...
      <view class="flex">
		...
		<!-- 订单疑问 -->
		<view class="item flex" style="justify-content: space-between; margin-bottom: 10px" @click="handleInputRemarks">
			<view class="text" style="margin: 0">订单备注</view>
			<view class="text" style="margin-left: 10px; flex: 1">{{ orderForm.message }}</view>
			<img src="../../static/images/common/chevron-states.png" alt="" />
		</view>
      ...
      <view class="flex">
		...
		<view class="guess">
			...
			<view class="guess-goods"><RecommendGoods :showTitle="false" :padding="0" :id="12"></RecommendGoods></view>
		</view>
		...
		<view class="footer">
			<view>
				总计：
				<text style="color: red">{{ '￥' + orderInfo.info.retailPrice * orderInfo.number }}</text>
			</view>
		...
		<uni-popup ref="orderRemarksRef">
			<uni-popup-dialog ref="inputClose" mode="input" type="input" title="请输入订单备注" :value="orderForm.message" @confirm="handleConfirmOrderRemarks"></uni-popup-dialog>
		</uni-popup>
	</view>
</template>
...
export default {
	...
	computed: {
		statusStyles() {
			return {
				background: this.status ? 'linear-gradient(270deg,  rgba(0,181,120,0), #07B9B9)' : 'linear-gradient(270deg,#FFFFFF,  #FA5151)'
			};
		},
