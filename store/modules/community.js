...
export default {
  ...
  mutations: {
    ...
    [CHANGE_HOME_STORE](state, currentAddress) {
      ...
      const findCurrentStore = communityShopList.find((item) => item.town.includes(currentAddress));

      state.homeCommunityStore = findCurrentStore ? findCurrentStore : communityShopList[0];
    },
  ...
  actions: {
    ...
    async getVipPackageList({ commit }, currentAddress) {
      const res = await queryDynamicDataApi({
        address: currentAddress && JSON.stringify(currentAddress) != '[]' ? currentAddress : '',
        correspondType: 2
      });
